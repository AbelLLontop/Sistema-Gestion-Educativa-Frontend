      ...
      <div className="absolute top-0 left-0 w-full h-screen bg-red bg-white/50">
        <div className="max-w-xl px-8 py-4 mx-auto mt-20 bg-white rounded-md shadow-md">
          ...
          <div className="flex justify-end gap-2 mt-8">
            <Button onClick={closeModal} mode={TypeButton.cancelar} />
            <Button onClick={saveData} mode={TypeButton.guardar} />
          </div>
        </div>
      </div>
      {loading && <Loading />}
    ...
    <>
      <div className="absolute top-0 left-0 w-full h-screen bg-red bg-white/50">
        <div className="max-w-xl px-8 py-4 mx-auto mt-20 bg-white rounded-md shadow-md">
          ...
          <div className="flex justify-end gap-2 mt-8">
            <Button onClick={closeModal} mode={TypeButton.cancelar} />
            <Button onClick={saveData} mode={TypeButton.eliminar} />
          </div>
        </div>
      </div>
      {loading && <Loading />}
  ...
  return (
    <section className="flex-1 p-8 bg-white rounded-md">
      <div className="flex justify-end gap-1">
        ...
        <Button onClick={btnRegistrar} mode={TypeButton.registrar} />
        {showModalRegistrar &&
          createPortal(
            <ModalRegistrar closeModal={() => setShowModalRegistrar(false)} />,
            document.body
          )}
        <Button onClick={btnRegistrar} mode={TypeButton.recargar} />
